...
internal class WhisperOnnxSpeechProcessor : ISpeechProcessor
{
    ...
    private readonly SpeechProcessorOptions options;

    public WhisperOnnxSpeechProcessor(string modelPath, SpeechProcessorOptions options)
    {
    ...
    public WhisperOnnxSpeechProcessor(byte[] modelBytes, SpeechProcessorOptions options)
    {
        var sessionOptions = new SessionOptions();
        sessionOptions.RegisterOrtExtensions();
        sessionOptions.AppendExecutionProvider_CPU();
    ...
    public async IAsyncEnumerable<ProcessedSpeechSegment> TranscribeAsync(IAudioSource source, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        {
            throw new NotSupportedException("Only 16 kHz audio is supported. Consider resampling before calling this processor.");
        }

        var samples = await source.GetSamplesAsync(0, cancellationToken: cancellationToken);

        var audioTensor = new DenseTensor<float>(memory: samples, dimensions: [1, samples.Length]);
        var timestampsEnableTensor = new DenseTensor<int>(new[] { options.RetrieveTokenDetails ? 1 : 0 }, [1]);

        var langCode = options.LanguageAutoDetect ? 54021 : GetLangId(options.Language.TwoLetterISOLanguageName);
        // 50359 = Transcribe, 50358 = Translate
        var decoderInputIds = new int[] { 50258, langCode, 50359 };
        ...
        foreach (var result in await Task.Run(
            () =>
            {
                ...
                var aaa = new List<ProcessedSpeechSegment>
                {
                    new() { Text = text }
                };
                return aaa;
    ...
    {
        if (langToId.TryGetValue(languageString, out var langId))
        {
            return langId;
        }
