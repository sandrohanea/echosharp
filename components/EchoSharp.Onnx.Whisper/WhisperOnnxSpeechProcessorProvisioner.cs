...
public class WhisperOnnxSpeechProcessorProvisioner(WhisperOnnxSpeechProcessorConfig config, ModelDownloader? modelDownloader = null) : ISpeechProcessorProvisioner
{
    ...
    public async Task<ISpeechProcessorFactory> ProvisionAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (config.ModelPath != null)
        {
            ...
            return await new WhisperOnnxSpeechProcessorFactory(modelOnnxPath).WarmUpAsync(config.WarmUp, cancellationToken);
        }
        ...
        return await new WhisperOnnxSpeechProcessorFactory(memoryModel.GetModelMemory(UnarchiverCopy.ModelName).ToArray())
