...
public sealed class SherpaOnnxSpeechProcessorFactory : ISpeechProcessorFactory
{
    ...
    public SherpaOnnxSpeechProcessorFactory(SherpaOnnxOfflineTranscriptorOptions options)
    {
    ...
    public SherpaOnnxSpeechProcessorFactory(SherpaOnnxOnlineTranscriptorOptions options)
    {
        // We need these providers as sample rate is required when recognizer is created, but we'll have it only when audio will be transcribed (later).
    ...
    public ISpeechProcessor Create(SpeechProcessorOptions options)
    {
        ...
        {
            throw new NotSupportedException("Only Transcript processing is supported by SherpaOnnxSpeechProcessors");
        }

        if (offlineProvider != null)
        {
            return new SherpaOnnxOfflineSpeechProcessor(offlineProvider, silenceProvider);
        }

        return new SherpaOnnxOnlineSpeechProcessor(onlineProvider!, silenceProvider);
    }
