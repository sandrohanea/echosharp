...
public class SherpaOnnxSpeechProcessorProvisioner(SherpaOnnxSpeechProcessorConfig config, ModelDownloader? modelDownloader = null) : ISpeechProcessorProvisioner
{
    ...
    public async Task<ISpeechProcessorFactory> ProvisionAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (config.Model is SherpaOnnxOfflineModel offlineModel)
        {
            ...
            return await new SherpaOnnxSpeechProcessorFactory(options).WarmUpAsync(config.WarmUp, cancellationToken);
        }
