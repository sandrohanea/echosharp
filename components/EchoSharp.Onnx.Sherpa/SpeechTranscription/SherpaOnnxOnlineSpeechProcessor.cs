...
internal sealed partial class SherpaOnnxOnlineSpeechProcessor(SherpaProvider<OnlineRecognizer> recognizers, SherpaProvider<SilenceAudioSource> silenceSources) : ISpeechProcessor
{
    ...
    [LibraryImport("sherpa-onnx-c-api", EntryPoint = "SherpaOnnxOnlineStreamAcceptWaveform")]
    private static partial void AcceptWaveform(IntPtr handle, int sampleRate, IntPtr samples, int n);
#else
    [DllImport("sherpa-onnx-c-api", EntryPoint = "SherpaOnnxOnlineStreamAcceptWaveform")]
    ...
    public async IAsyncEnumerable<ProcessedSpeechSegment> TranscribeAsync(IAudioSource source, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        var transcriptSegment = new ProcessedSpeechSegment()
        {
            Text = text
        };
        ...
        if (tokens != null || timeStamps != null)
        {
            var tokensLength = (tokens != null && timeStamps != null)
                ...
                : tokens != null
                        ? tokens.Length
                        : timeStamps!.Length;
            var tokensList = new List<ProcessedSpeechToken>(tokensLength);
            var startTime = TimeSpan.Zero;

            for (var i = 1; i < tokensLength; i++)
            {
                tokensList.Add(new ProcessedSpeechToken()
                {
                    Text = tokens?[i - 1],
    ...
    private static unsafe void AddWaveData(OnlineStream stream, Memory<float> samples)
    {
