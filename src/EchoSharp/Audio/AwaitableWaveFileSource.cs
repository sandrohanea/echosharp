...
/// </remarks>
public class AwaitableWaveFileSource(bool storeSamples = true,
    ...
    : AwaitableAudioSource(storeSamples, storeBytes, initialSizeFloats, initialSizeBytes, aggregationStrategy)
{
    ...
    public void WriteData(ReadOnlyMemory<byte> data)
    {
        ...
        if (!IsInitialized)
        {
            ...
            if (headerParseResult.IsIncomplete)
            {
                ...
                headerChunks = new(headerChunks.ToArray());
                return;
            }
    ...
    private void ProcessSamples(ReadOnlyMemory<byte> sampleData)
    {
        ...
        {
            sampleDataChunks = new(sampleData);
        }
