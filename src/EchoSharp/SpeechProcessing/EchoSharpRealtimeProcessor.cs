...
internal class EchoSharpRealtimeProcessor(
    ISpeechProcessorFactory speechTranscriptorFactory,
    IVadDetector vadDetector,
    ISpeechProcessorFactory? recognizingSpeechProcessorFactory,
    RealtimeSpeechProcessorOptions options,
    EchoSharpRealtimeOptions echoSharpRealtimeOptions)
    : IRealtimeSpeechProcessor
{
    public async IAsyncEnumerable<IRealtimeRecognitionEvent> TranscribeAsync(IAwaitableAudioSource source, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (!source.IsFlushed)
        {
            ...
            {
                var transcribingEvents = TranscribeSegments(
                    recognizingSpeechProcessorFactory ?? speechTranscriptorFactory,
                    source,
                    processedDuration,
    ...
    private async IAsyncEnumerable<ProcessedSpeechSegment> TranscribeSegments(ISpeechProcessorFactory transcriptorFactory,
                                                                         IAudioSource source,
